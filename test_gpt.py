...
from nn_sdk import *
config = {
    "model_dir": r'D:\onnx\gpt\best.onnx',
    "aes":{
        "use":False,
...
tokenizer = FullTokenizer(vocab_file=r'D:\onnx\gpt\vocab.txt',do_lower_case=True)
...
if sdk_inf.valid():
    def fn_predict_call(fn_args,
        ...
        net_stage = 0
        ret, outputs = sdk_inf.process(net_stage, encoder_decoder_args[0][0].astype(np.int64), encoder_decoder_args[0][1].astype(np.int64))
        logits = outputs[0]
        return logits
    ...
    result = generate.decode(text='[unused13]' + '<t>',
                    end_tokens=['$','[SEP]'],
                    max_length=100,
                    num=10,
