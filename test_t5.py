...
from nn_sdk import *
config = {
    "model_dir": r'D:\onnx\t5\best.onnx',
    "aes":{
        "use":False,
    ...
    "graph": [
        {
            ...
            "input": [
                ...
                {"node":"attention_mask","dtype":"int64",},
                {"node":"decoder_input_ids","dtype": "int64",},
                {"node":"decoder_attention_mask","dtype":"int64",},
            ],
            "output": [
                {
...
tokenizer = FullTokenizer(vocab_file=r'D:\onnx\t5\vocab.txt',do_lower_case=True)
...
if sdk_inf.valid():
    def fn_predict_call(fn_args,
        ...
        net_stage = 0
        ret, outputs = sdk_inf.process(net_stage, encoder_decoder_args[0][0].astype(np.int64),
                                       encoder_decoder_args[0][1].astype(np.int64),
                                       encoder_decoder_args[1][0].astype(np.int64),
                                       encoder_decoder_args[1][1].astype(np.int64),
                                       )
        logits = outputs[0]
        return logits


    generate = GenerateMinMax(tokenizer,with_decoder=True,
                              bos_token='[CLS]',
                              eos_token='[SEP]',
                              unk_token='[UNK]',
                              pad_token='[PAD]'
